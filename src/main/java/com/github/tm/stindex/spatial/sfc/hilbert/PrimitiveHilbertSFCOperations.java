 ...
 * as BigInteger, but can only support up to certain levels of precision. For {@code getIndex()}
 * operations it is currently used if no single dimension is more than 48 bits of precision, and for
 * query decomposition it is currently used if the total precision is <= 62 bits.
 *
 * @author Yu Liebing
 */
public class PrimitiveHilbertSFCOperations implements HilbertSFCOperations {
  ...
  public byte[] convertToHilbert(
          ...
          final int dimNum,
          final int res) {
    final BitVector[] bitVectors = new BitVector[dimNum];
    ...
    for (int i = 0; i < dimNum; i++) {
      bitVectors[i] =
              BitVectorFactories.OPTIMAL.apply(res);
      bitVectors[i].copyFrom(coordinates[i]);
    }
   ...
   *
   * @param values n-dimensional point to transform to a point on the hilbert SFC
   * @return point on hilbert SFC
   */
  public BitVector convertToHilbert(
  ...
  public long[] indicesFromHilbert(byte[] hilbertValue,
                                   ...
                                   final int dimNum,
                                   final int res) {
    final BitVector[] perDimensionBitVectors = new BitVector[dimNum];
    ...
    final long[] retVal = new long[dimNum];
  ...
  public RangeDecomposition decomposeRange(
    ...
    for (int i = 0; i < hilbertRanges.size(); i++) {
      ...
      sfcRanges[i] = new ByteArrayRange(new SpatialByteArray(start, res),
                                        new SpatialByteArray(end, res));
    }
