...
public class ByteArrayUtils {
  ...
  /**
   * swap the endian of each byte to facilitate processing with {@code BitSet}
   * */
  ...
  /**
   * swap the endian of a byte to facilitate processing with {@code BitSet}
   * e.g. 0b1000_1011 --> 0b1101_0001
   * */
  ...
  /**
   * Get the common bit num of two byte array.
   * */
  public static int commonBitsNum(byte[] b1 , byte[] b2) {
    ...
    int commonBitsNum = 0;
    int len = b1.length < b2.length ? b1.length * 8 : b2.length * 8;
    for (int i = 0; i < len; i++) {
  ...
  public static ByteArray commonPrefix(byte[] b1 , byte[] b2) {
    byte[] tmp = b1.length > b2.length ? b2 : b1;
    int commonPrefix = commonBitsNum(b1, b2);
    final byte[] littleEndianBytes = swapEndianFormat(tmp);
